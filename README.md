<h1 align="center">
  Hi there, I’m Abobakr Safwat
</h1>

<p align="center">
Mechanical Engineer • Data Engineering Padawan • Java & Python Developer
</p>


---
## 🕵️ About Me
I’m a Mechanical Engineer specializing in Remotely Operated Vehicles (ROVs), currently studying at the Faculty of Computers and Data Science, Alexandria University.

Somewhere between designing pressure-resistant hardware and writing Object-Oriented code, I realized something important: hardware is awesome, but the data it generates is where the real magic happens. Now, I am bringing my systems-thinking mindset from the physical world into the digital world of Data Engineering.

I know my way around Java, Python, and SQL, and I'm actively expanding my toolkit to architect reliable, scalable data systems. If I can engineer a machine to survive the crushing pressure of the ocean, I can definitely handle complex data transformations!

📄 **Resume:** [View PDF](https://drive.google.com/file/d/1byd0jVKlD5gXtpiAUizVRPpoef9yCKgl/view?usp=sharing)

---
## 🤝 Catch Me Online
<p align="center">
<a href="https://www.linkedin.com/in/abobakrms"><img src="https://skillicons.dev/icons?i=linkedin" height="50" />
<a href="mailto:abobakrms.rm@gmail.com"><img src="https://skillicons.dev/icons?i=gmail" height="50" />
<a href="https://github.com/Abobakrms"><img src="https://skillicons.dev/icons?i=github" height="50" />
</p>

---
## 🏗️ The Engineering Toolbox
#### 💻 Software & Data 
<p align="center">
<img src="https://th.bing.com/th/id/R.aa9268c2f03767c62454ac45d5459cd7?rik=vZmmENhShEgOqA&pid=ImgRaw&r=0" height="50" />
<img src="https://logos-world.net/wp-content/uploads/2022/07/Java-Logo.png" height ="60" />
<img src="https://logodix.com/logo/542147.png" height ="50"/>
</p>

#### ⚙️ Mechanical & Domain Expertise
<p align="center">
<img src="https://scontent.fcai21-3.fna.fbcdn.net/v/t39.30808-6/340076686_594975785904369_2474232304390438263_n.png?_nc_cat=100&ccb=1-7&_nc_sid=1d70fc&_nc_ohc=kffaKvIpjucQ7kNvwE7EpRY&_nc_oc=AdlpEAN016TvuW7UvkbqOh1ISmpw5gm3FwTIJoQcUbDLzZwfGjuaLpvpgYwAJjjbD28&_nc_zt=23&_nc_ht=scontent.fcai21-3.fna&_nc_gid=rz25VC56KoWzclpkjIqj-g&oh=00_Afs2SBmGajrTYBLJEtI44_v4WVsDozh4Kq0L3RbYMrNT1Q&oe=69A56403" height="60" />
<img src="https://i.postimg.cc/bNgjg9zH/Draven-202400.png" height="60"/>

</p>
<h3>🌊 Why hire an ROV Engineer for Data?</h3> 
<p>
<b>Fault Tolerance: </b>I am used to building systems where a failure means losing expensive hardware at the bottom of the sea. I treat data infrastructure with the same level of care.

<b>First-Principles Thinking: </b>I don't just memorize syntax; my background in physics and mechanics means I naturally break complex software problems down to their most basic, solvable parts.

<b>Telemetry & Sensors: </b>I understand the physical reality of how data is generated in the real world before it ever hits a database.

⭐️ <b>Currently open to opportunities in Data Engineering, Backend Development, and System Architecture.</b>
</p>
